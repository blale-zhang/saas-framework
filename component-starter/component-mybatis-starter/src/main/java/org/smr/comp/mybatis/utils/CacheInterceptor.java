...
public class CacheInterceptor implements Interceptor {
    ...
    private static String defaultPageSqlId = ".*Page$"; // �?要拦截的ID(正则匹配)
    private static String pageSqlId = ""; // �?要拦截的ID(正则匹配)
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        while (metaExecutor.hasGetter("h")) {
            ...
            metaExecutor = MetaObject.forObject(object, DEFAULT_OBJECT_FACTORY, DEFAULT_OBJECT_WRAPPER_FACTORY, null);
        }
        // 分离�?后一个代理对象的目标�?
                    ...
                    if (!(Boolean) metaExecutor.getValue("dirty")) {
                        ...
                        try {
                            ...
                            if (value != null) {
                                ...
                                Pager cachedPage = (Pager) cachedMap.get("page");
                                Pager originalPage = (Pager) metaParameter.getValue("page");
                                if (null != originalPage) {
                                    Long totalCount = cachedPage.getTotalCount();
    ...
    private CacheKey createCacheKey(MappedStatement ms, Object parameterObject, RowBounds rowBounds, BoundSql boundSql) {
        ...
        // 当需要分页查询时，将page参数里的当前页和每页数加到cachekey�?
        if (ms.getId().matches(pageSqlId) && metaObject.hasGetter("page")) {
            Pager page = (Pager) metaObject.getValue("page");
            if (null != page) {
